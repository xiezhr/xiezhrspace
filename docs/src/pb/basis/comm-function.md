...
- `fileaccess`-->`FileAccess`枚举类型，可选项，指定文件访问方式。
  ...
  - [x] `Read!` - 缺省值，只读方式，这样打开的文件只能进行读操作；
  - [x] `Write!` - 只写方式，这样打开的文件只能进行写操作

- `filelock`-->`FileLock`枚举类型，可选项，指定文件加锁方式

  - [x] `n LockReadWrite!` - 缺省值，只有打开该文件的用户能够访问该文件，其它用 户对该文件的访问均被拒
  - [x] `n LockRead!` - 只有打开该文件的用户能够读该文件，但其它任何用户均可写该文件
  - [x] `n LockWrite!` - 只有打开该文件的用户能够写该文件，但其它任何用户均可读该文件
  - [x] `n Shared!` - 所有用户均可读写该文件

- `writemode`-->`WriteMode`枚举类型，可选项

  - [x] Write!  该参数指定在指定文件已经存在时数据的添加方式
  - [x] Append! - 缺省值，将数据添加到原文件尾部
...
- `origin`-->`eekType`枚举类型，指定从哪里开始移动文件指针，即指针移动的基准
  - [x] `FromBeginning!` - 缺省值，从文件开头移动指针
  - [x] `FromCurrent! `- 从当前位置移动文件指针
  - [x] `FromEnd! `- 从文件结尾处移动文件指针 
...
- `tab1`-->`Integer`类型，**可选项**，指定文本开始打印的位置，在开始打印字符串之前将打印光标移动到该位置，
  ...
  打印光标的当前位置开始打印
- `string`-->`string`类型，指定要打印的字符串。如果字符串中包括回车换行字符（~r~n），那么该字符串将被
...
- `italic`：`boolean`类型，指定是否使用斜体样式。有效取值为：
  - [x] TRUE - 使用斜体样式
  - [x] FALSE - 不使用斜体样式 缺省值为FALSE  

`underline`-->`boolean`类型，指定是否加下划线。

- [x] TRUE - 加下划线；
- [x] FALSE - 不加下划线 缺省值为FALSE  
